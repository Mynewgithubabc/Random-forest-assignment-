...
```{r}
TNR <- (sum(m[,2]) + (sum(m[,3]))) / (sum(m[2,]) + sum(m[3,]))
TNR
```
...
```{r}
TNR <- ((sum(m[,1]) + sum(m[,3]))) / (sum(m[1,]) + sum(m[3,]))
TNR
...
```{r}
TNR <- ((sum(m[,1]) + sum(m[,2])) / (sum(m[2,]) + sum(m[1,])))
TNR
...
for (n in n_values) {
  ...
  N <- nrow(penguins_selected_variables)  # Assuming 'penguins' is your dataset
...
for (n in n_values) {
  ...
  M <- 50
  for (i in 1:M) {
    # Generate n noisy arrays
    noisy_arrays <- lapply(1:n, function(j) rnorm(N, 0, 1))
...
```{r}
# Get the total number of rows in covertype_data
...
train_indices <- sample(all_indices, 1000, replace = FALSE)

# Exclude train_indices from the pool of indices
remaining_indices <- setdiff(all_indices, train_indices)

# Sample indices for Ntest from the remaining indices
test_indices <- sample(remaining_indices, 1000, replace = FALSE)

# Create data_train and Ntest using the sampled indices
