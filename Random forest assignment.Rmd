...
I used data.generator.1 to generate 200 data points.
I then ran a random forest model to predict y given the values of x1, x2 and x3 that I generated. I am feeding the data and the outcomes to our model so that my model learns how to predict the outcomes from new data based on this. 
...
I forced a large number of trees (1000) and calculated the OOB error rates for each number of trees.
...
After having set the number of trees in the random forest back to 500, the default, I am now using the confusion matrix to calculate the measures of predictive power, sensitivity, specificity and accuracy.
...
The True negative rate is the number of correctly predicted entires for "no" divided by the sum of all entries that are actually "0" / no, 
In our case the true negative rate is calculated by the number of correctly predicted entries for 0 divided by the sum of all entries that are actually 'No".
...
I then repeat the same analysis for data generator 2. I first generate 200 data points using data.generator.2.
...
I again force a large number of trees, setting ntree to 1000, and calculated the OOB error rate for each number of trees.
...
Having set my ntree back to the default, 500, I will now calculate sensitivity, specificity and accuracy for data.generator.2: 
...
I am going to use a random forest model to predict the species of penguins in the "palmerpenguins" dataset, solely using the physical attributes of the penguins(bill length, bill depth, flipper length, and body mass). 
...
I have loaded the data on palmerpenguins from the package "palmerpenguins" which was installed above. I saved the raw data as a csv and performed a number of cleaning steps which involved removing any rows with NA values, and removing the variables sex, year and island, since these are not physical attributes and are therefore not relevant to the model. 


Creating histograms for each variable using the ggplot funciton, stratifying by species. Each figure will contain 3 histograms, one per species. 
...
head(rf$err.rate)
```
In order to find the appropriate number for ntree, I used ntree = 1000 to force a large number of trees and ran the random forest again. I then calculated the OOB error rate for each number of trees. 
...
Mtry is the number of variables used to decide the species for each tree that is used in the random forest model. The default value of mtry is the square root of the total number of variables in the dataset. I forced a large number of trees (ntree = 1000) and used a for loop to run a random forest for every possible value of mtry (1,2,3 and 4) and stored these results. I then plotted the OOB error rate for each number of trees, for each value of mtry.  
...
```
Since I have decided to keep the default parameters, I reran the random forest model with the default parameters. 

Finding the most important variables: 

The random forest will quantify how important each variable was for the predicting what species each individual datapoint in the dataset belongs to.  We can extract and plot this information from the forest, and plot the importance information: 

```{r echo=FALSE, include=TRUE, fig.width=10, fig.height=4}
...
Repeating these calculations for Chinstrap and Gentoo:
...
I will next use the ggplot function to create scatter plots of predictor variables against eachother (colouring points by penguin species) to explain why some variables are more important than others in predicting penguin species.
...
I have created n = 0, 1, 5, 10, 20, 50, 75 and 100 arrays of length N, where N is the number of points in the cleaned dataset. These points were generated using random draws from the normal distribution, (rnorm(0,1)). I then appended these new columns of data to my original dataset using the cbind function. I set ntree to 2000, and used default values for mtry. I ran a random forest algorithm on each augmented dataset and repeated this 50 times for each value of n, and calculated the accuracy of the random forest (using the confusion matrix generated), for each value of n. 
...
I then used the ggplot function to plot the mean accuracy as a function of n. 
...
results_df <- data.frame( #creating a new data frame called results_df 
  class = 1:num_classes, #storing the variable class 
  accuracy = accuracy,
  sensitivity = sensitivity,
  specificity = specificity #storing the variables accuracy, sensitivity and specificity we just calculated 
)
...
Firstly, class 5 has a small sample size of 158 in data_train (the dataset used to train the random forest model). If this class is underrepresented in data_train this could lead to higher rates of miss-classification. 
